...
use DateTimeZone;
use DataTables;
use App\{TaxDocument, CreditNoteDetail, EntityMasterdata, Company, BranchOffice, CorrelativeDocument, Invoice, Product, CountryTax};
...
class CreditNoteController extends Controller
{
    ...
    public function indexDt($id){
      ...
      $creditNotes = DB::table('tax_document')
      ...
      return DataTables::query($creditNotes)->make(true);
    }
    ...
    public function store(Request $request){
        ...
        foreach ($request['creditNoteLine'] as $line) {
          $aux = json_decode($line);                  

          $creditNoteLine = creditNoteDetail::create([ 
            'quantity'    => $aux->quantity,
            'unit_price'  => $aux->unit_price,            
            'taxdocument_id'  => $taxdocument->id,
    ...
    public function getCreditNoteXML($id){

      $creditNote = TaxDocument::where('id', $id)->select(['xml_generated', 'auth_code'])->First();
      return $creditNote;
    }

    public function getCreditNote($id) {                
        $creditNote = TaxDocument::with('creditNoteLine', 'branch')->select("tax_document.*")->where('tax_document.id', $id)->First();

        $environment = $creditNote->branch->environment_type; 
        $emission    = $creditNote->branch->emission_type;               

        $environment_type = EntityMasterdata::where('id', $environment)->First();        
        $emission_type    = EntityMasterdata::where('id', $emission)->First();
        $document_type    = EntityMasterdata::where('id', $creditNote->document_type_id)->First();
        
        $date       = date_create($creditNote->emission_date);
        $dateFormat = date_format($date, "d/m/Y");

        $arr = [];
        $arr['id']               = $creditNote->id;
        $arr['principal_code']   = $creditNote->principal_code;
        $arr['referral_code']    = $creditNote->referral_code;
        $arr['emission_date']    = $dateFormat;
        $arr['amount']           = $creditNote->amount;
        $arr['concept']          = $creditNote->concept;
        $arr['xml_generated']    = $creditNote->xml_generated;
        $arr['auth_code']        = $creditNote->auth_code;
        $arr['auth_date']        = $creditNote->auth_date;
        $arr['emission_type']    = $emission_type->code;
        $arr['environment_type'] = $environment_type->code;
        $arr['invoice_id']       = $creditNote->invoice_id;
        $arr['document_type_id'] = $document_type->code;


        $invoice = Invoice::with('client', 'invoiceLine')->select("invoice.*")->where('invoice.id', $creditNote->invoice_id)->First(); 
        $date       = date_create($invoice->invoice_date);
        $dateFormat = date_format($date, "d/m/Y");        
        $arr['modifiedDocumentEmissionDate'] = $dateFormat;

        $documentLine = $creditNote->creditNoteLine;        
        ...
        foreach ($documentLine as $line) {        
            ...
            $country_tax = CountryTax::where('id', $line->country_tax_id)->First();
            $tax_percentage = EntityMasterdata::where('id', $country_tax->tax_percentage_id)->First()->code;

            $SUBTOTAL += $line->quantity * $line->unit_price;

            if($tax_percentage == $IVA_12){
                $SUBTOTAL_12  += ($line->unit_price * $line->quantity);
                $TOTAL_12     += ($line->unit_price * $line->quantity) * ($country_tax->value / 100);
            }
            else{
                if($tax_percentage == $IVA_0){
                    $SUBTOTAL_0 += ($line->unit_price * $line->quantity);
                }
                else{ 
                    if($tax_percentage == $IVA_14){
                        $SUBTOTAL_14 += ($line->unit_price * $line->quantity);
                        $TOTAL_14    += ($line->unit_price * $line->quantity) * ($country_tax->value / 100);
                    }
                    else{ 
                        if($tax_percentage == $IVA_NO_OBJETO){
                            $SUBTOTAL_NO_OBJETO += ($line->unit_price * $line->quantity);
                        }
                        else{ 
                            if($tax_percentage == $IVA_EXENTO){
                                $SUBTOTAL_EXENTO += ($line->unit_price * $line->quantity);
                            }
                        }
                    }
                }
    ...
    public function taxBase($creditNoteLine, $taxId){
        $taxBase = 0;

        foreach ($creditNoteLine as $tax) {
            if($tax['countryTax'] == $taxId){
                $taxBase += $tax['line_sub_total'];
            }
        }
        return $taxBase;
    }
